    ...
    def choose_best_action(self, actions, env):
        ...
        if actions:
            ...
            for i, action in enumerate(actions):
                observation, reward, done, info = env.step_forward(action)
                values[i] = self.net(observation).detach().numpy()
...
def evaluate_agents(agents, env, n_episodes):
    ...
    for episode in range(n_episodes):
        ...
        for i in count():
            ...
            observation_next, reward, done, winner = env.step_forward(action)
